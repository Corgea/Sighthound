...
// Exercises `file_types.include_patterns` / `exclude_patterns` glob matching
// through the public scanner surface. The rules below are search-mode rules
// targeting Python; the include/exclude patterns gate which files each rule
// actually inspects.
...
mod file_pattern_tests {
    ...
    fn test_include_patterns_functionality() {
        ...
        let test_only_findings: Vec<_> = findings
            .iter()
            .filter(|f| f.finding_type == "test_only")
            .collect();
        let universal_findings: Vec<_> = findings
            .iter()
        ...
        assert!(
            ...
            "universal_function should be found in test file"
        );
    }
    ...
    fn test_exclude_patterns_functionality() {
        ...
        let safe_findings: Vec<_> = findings.iter().filter(|f| f.file.contains("safe_file")).collect();
    ...
    fn test_combined_include_exclude_patterns() {
        ...
        assert_eq!(
            config_findings.len(),
            2,
            "Should find config issues in 2 files (app_config and settings)"
        );
    ...
    #[test]
    fn test_glob_pattern_configuration() {
        let rules_file = create_test_rules_with_patterns();

        let rules = Rules::load_from_file(rules_file.path().to_str().unwrap())
            .expect("Failed to load rules");

        // Glob-matching logic itself is private; this test pins down the
        // deserialized `file_types` configuration so a regression in RON
        // parsing of include/exclude lists would surface immediately.
